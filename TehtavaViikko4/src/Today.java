...
public class Today {
    ...
    public static void main(String[] args) {
        ...
        Today app = new Today();
        if(args.length > 0) {
            if(args[0].matches("--\\d{2}-\\d{2}")){
                System.out.println("Date: " + args[0]);
            } else {
                System.out.println("Use format --MM-DD");
                return;
            }
               //make sure the date is valid otherwise close the program
            if(args[1].matches("\\w+/\\w+")){
                System.out.println("Category: " + args[1]);
            } else {
                System.out.println("Use format primary/secondary");
                return ;
            }
        }
        else {
            System.out.println("Please provide a date and a category");
            return;
        }
        ...
        app.addEvents();
        app.report();
        //Make a list of found events 
        List<Event> foundEvents = new ArrayList<>();
        LocalDate askedDate = LocalDate.parse("2025-" + args[0].substring(2,4)+ "-" + args[0].substring(5,7));
        Category category = Category.parse(args[1]);

        for (Event event : app.events) {
            if(app.isSameDate(askedDate, event.getDate() ) && event.getCategory().equals(category)){
            foundEvents.add(event);
            }
        }
        if(foundEvents.isEmpty()){
            System.out.println("No events found for the date and category");
            return;
        }
        //Sort the list of found events with their year
        Collections.sort(foundEvents, (Event e1, Event e2) -> e1.getDate().compareTo(e2.getDate()));
    ...
    private void addEvents() {
        ...
        macOSEvents.add(makeEvent("2010-04-29", "Mac OS X 10.4 Tiger released", "apple/macos"));
        macOSEvents.add(makeEvent("2006-04-29", "Mac OS X 10.4 Tiger released", "apple/macos"));
        macOSEvents.add(makeEvent("2007-04-29", "Mac OS X 10.4 Tiger released", "apple/macos"));
